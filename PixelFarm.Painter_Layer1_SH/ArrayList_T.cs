...
namespace PixelFarm.CpuBlit
{
    public readonly struct ArrayListSegment<T>
    {
        ...
        public void WriteTo(ArrayList<T> another)
        {
    ...
    public sealed class ArrayList<T>
    {
        ...
        public void Append(ArrayListSegment<T> segment)
        {
            EnsureSpaceForAppend(segment.len);
            ArrayListSegment<T>.UnsafeGetInternalArr(segment, out T[] src);

            System.Array.Copy(src, segment.beginAt, _internalArray, _currentSize, segment.len);

            _currentSize += segment.len;
        }
